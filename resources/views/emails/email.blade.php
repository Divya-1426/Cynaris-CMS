	        ...
	        <td class="bg_dark email-section text-center">
	        	<div class="heading-section heading-section-white">
	          	...
	          	<address>
	          		...
	          		<strong>{{ __('email.name') }}:</strong> {{ $data['row']->name }}<br/>
	          		@endif
	          		@if(!empty($data['id_type']) && !empty($data['order_id']))
	          		<strong>{{ $data['id_type'] }}:</strong> #{{ $data['order_id'] }}<br/>
	          		@endif
	          		@if(!empty($data['amount']))
	          		<strong>{{ __('email.bill') }}:</strong> {{ $data['amount'] }} {{ __('common.currency') }}<br/>
	          		@endif
	          	</address>

	          	@php
	          	$services_array = array();
	          	foreach($data['row']->services as $service){
                ...
                $replace = array('<b>'.$data['row']->name.'</b>', '<b>'.$data['row']->email.'</b>', '<b>'.$data['row']->phone.'</b>', '<b>'.$data['row']->address.'</b>', '<b>'.$data['row']->city.'</b>', '<b>'.$data['row']->company.'</b>', '<b>'.$services_list.'</b>');

                $message_body = str_replace($search, $replace, $data['message']);
                @endphp

				<p>{!! strip_tags($message_body, '<p><a><b><i><u><strong><br><table><tr><th><td><ul><ol><li><h1><h2><h3><h4><h5><h6><del><ins><sup><sub><pre>') !!}</p>
				<br/>
          		<!-- Content Area -->
	        	</div>
	        </td>
